"""Run the L1 synthetic collection (structural, LLM-free) and report the gate.

Each case is a fully structured theory/query pair with an independently written
expected verdict. The runner builds the engine models directly and calls
``verify`` + ``build_answer``; no extraction, no LLM, no provider variance.

Usage:
    uv run python -m evals.l1_synthetic [--sample PATH] [--ids a,b] [--limit N]
...
SAMPLE = ROOT / "data" / "l1_synthetic.jsonl"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run the L1 synthetic gate.")
